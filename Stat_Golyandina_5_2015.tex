...
\subtitle{Черновой конспект, присылайте найденные опечатки}


\author{Мат-Мех, ПМИ, СМ--СМ, конспект семестров 5--8\\
Лекции Голяндиной Н.Э. \thanks{Первоначальный набор текста сделан Дмитрием Зотиковым}
}
...
\begin{rem*}
Если среднее $\E\xi=a$ известно, то дополнительно вводится $s_{a}^{2}:=
\frac{1}{n}\sum_{i=1}^{n}\left(x_{i}-a\right)^{2}$.
\begin{comment}
...
Пусть $\Pcal$ \emph{симметрично} \emph{--- }в этом случае $\med\xi=\E \xi$
и имеет смысл сравнить две оценки одной и той же характеристики, выборочное среднее и выборочную медиану. %
\begin{comment}
...
$\bar{x}$, и этим лучше. Это легко увидеть, устремив одно из значений к бесконечности. Выборочное среднее тоже устремится к бесконечности, а выборочная медиана либо не изменится, либо почти не изменится.
\end{rem*}
\end{example*}
\begin{comment}
Достаточность
...
При проверки гипотезы (уже после эксперимента), если выборка попала в критическую область $\mathcal{A}^{(\text{крит})}_\alpha$, то нулевая гипотеза отвергается, а если в доверительную $\mathcal{A}^{(\text{дов})}_\alpha$, то не отвергается (важно, что именно нет основания отвергнуть, а принять нельзя).
...
Поэтому критическая область $\Ascr_{\text{крит}}^{(\a)}$ выбирается так, чтобы
$\alpha_I=\P_{H_0} (t\in \Ascr_{\text{крит}}^{(\a)}) = \alpha$. Заметим, что мы поменяли обозначение для областей, потому что это разбиение значений статистики критерия, а ранее это было разбиение множества значений выборки.
...
\begin{itemize}
\item $\P(t\in \Ascr_{\text{крит}}^{(\a)} )= \alpha$.

\item Если альтернативная гипотеза $H_{1}$ (см. про нее в след.разделе) не конкретизирована, то $\Ascr_{\text{крит}}^{(\alpha)}$ следует выбрать
...
от 0 так, что для $\Ascr_{\text{крит}}^{(\alpha)}=(-\infty,-t_{\a})\cup(t_{\a},\infty)$
\[
...
\end{example*}
\item Если $H_{1}$ известна, то $\Ascr_{\text{крит}}^{(\alpha)}$ выбирается так,
...
\begin{defn*}
Критерий называется \emph{состоятельным} против альтернативы $H_{1}$, если $\b\xrightarrow{n\to \infty} 1$.\end{defn*}
\begin{rem*}
...
\]
 В то же время, поскольку при верной $H_{1}$, $\E\bar{\mathbf{x}}=1/n\cdot\sum_{i=1}^{n}\E x_{i}=n/n\cdot a_{1}$
то
\[
...
Чтобы минимизировать $\a_{\mathrm{II}}$, логично определить $\Ascr_{\text{крит}}^{(\alpha)}$
...
попадания $t$ в $\Ascr_{\text{крит}}^{(\alpha)}$), а при меньших --- не
отвергается.
\end{defn*}
...
$s^{2}/\sigma_{0}^{2}$ (или $s_{{a}}^2/\sigma_{0}^{2}$ если ${a}$
...
\begin{prop*}
Пусть $\E\xi={a}<\infty$. При условии $\xi\sim \N(a,\sigma^2)$ используется
...
\begin{prop*}
Пусть $\E\xi$ неизвестно. При условии $\xi\sim \N(a,\sigma^2)$ используется
следующая статистика:
\[
...
роль уровня значимости. По определению доверительного интервала, $\P(\theta\in\left[b_1(\mathbf{x}),b_1(\mathbf{x})\right])=\gamma.$
Тогда
\[
\P(\theta\in\left[b_1(\mathbf{x}),b_2(\mathbf{x})\right])=
\gamma=1-\a \implies\a=1-\P(\theta\in\left[b_1(\mathbf{x}),b_2(\mathbf{x})\right])=
...
Пусть $n=4040$, $\#\mathrm{H}=2048,\ \#\mathrm{T}=1992$. Проверим
$H_{0}:\Pcal=\Ber(0.5)$ с $\a=0.1$. Условия критерия выполняются,
поэтому посчитаем
\[
T=\frac{(2048-2020)^{2}}{2020}+\frac{(1992-2020)^{2}}{2020}=\frac{28^{2}+28^{2}}{2020}\approx0.78,
\]
откуда
\[
...
\begin{rem*}
Если нужно подстраховаться от подгонки (искусственно составленных под гипотезу выборок), то критическую область можно выбрать с двух сторон, слева и справа. Например, чтобы отверглась гипотеза с $p=0.5$ для альтернирующей (и явно не
случайной) последовательности $\mathbf{x}=(0,1,0,1,\ldots)$, когда
$T=0$.
...
Альтернатива только одна: $H_{1}:\xi\not\sim\Pcal_{0}$; $\Ascr_{\text{крит}}^{(\alpha)}=(\qnt_{\text{K-S}}(1-\a),\infty).$\end{claim*}
\begin{rem*}
...
\paragraph{Разбиение}
\begin{description}
\item [{$H_{1}:\E\xi_{1}\neq\E\xi_{2}$}] $\Ascr_{\text{крит}}^{(\alpha)}=\RR\setminus\left(\qnt_{\t(n_{1}+n_{2}-2)}(\a/2),\qnt_{\t(n_{1}+n_{2}-2)}(1-\a/2)\right)$
\item [{$H_{1}:\E\xi_{1}>\E\xi_{2}$}] $\Ascr_{\text{крит}}^{(\alpha)}=(\qnt_{\t(n_{1}+n_{2}-2)}(1-\a),\infty)$
\item [{$H_{1}:\E\xi_{1}<\E\xi_{2}$}] $\Ascr_{\text{крит}}^{(\alpha)}=(-\infty,\qnt_{\t(n_{1}+n_{2}-2)}\a)$
\end{description}
...
\paragraph{Разбиение}
\begin{description}
\item [{$H_{1}:\E\xi_{1}\neq\E\xi_{2}$}] $\Ascr_{\text{крит}}^{(\alpha)}=\RR\setminus\left(z_{\a/2},z_{1-\a/2}\right)$
\item [{$H_{1}:\E\xi_{1}>\E\xi_{2}$}] $\Ascr_{\text{крит}}^{(\alpha)}=(z_{1-\a},\infty)$
\item [{$H_{1}:\E\xi_{1}<\E\xi_{2}$}] $\Ascr_{\text{крит}}^{(\alpha)}=(-\infty,z_{\a})$
\end{description}
...
что выдают один и тот же $p$-value.
%Тем не менее, проверяют они разные гипотезы ($\E\xi$ не то же, что $\med\xi$).
\end{rem*}

Если функция из какого-то пакета выдает p-value, обратите внимание на то, это p-value для односторонней альтернативы или двухсторонней. p-value для одностороннего варианта в два раза меньше p-value для двухстороннего. При этом имеется в виду конкретная односторонняя альтернатива, которая лежит с той же стороны, что и статистика критерия.
...
тогда
\[
\widehat{\tau}=\frac{\#(\text{одинаково упорядоченных пар})-\#(\text{по-разному упорядоченных пар})}{\#(\text{комбинаций пар})},
\]
...
$H_{0}:\tau=0$ и если $H_{0}$ отвергается, то тренд присутствует.
\begin{comment}
@xio: FIXME: прояснить!
\end{comment}
\begin{comment}
...
\[
\underbrace{\sum_{i=1}^{k}\sum_{j=1}^{n_{i}}(y_{ij}-\bar{y})^{2}}_{Q=n\widehat{\D\eta}}=
\underbrace{\sum_{i=1}^{k}n_{i}\left(\bar{y}_{i}-\bar{y}\right)^{2}}_{Q_{1}=
n\widehat{\D\E(\eta\mid\xi)}}+\underbrace{\sum_{i=1}^{k}\sum_{j=1}^{n_{i}}(y_{ij}-\bar{y}_{i})^{2}}_{Q_{2}},
\]
...
\begin{rem*}
Результат эксперимента сложно повторить, потому что при упорядочивании гипотезы
могут перемешиваться и поэтому поправки для каждого сравнения поменяются.
\end{rem*}
%\begin{prop*}
%$\FWER\leq\a$.\end{prop*}
%\begin{proof}
...
\section{Контроль за FDR}
Так как при контроле за FWER (за вероятностью того, что FP>=1 в таблице \ref{tab:TPFPTNFN}, т.е. будет хотя бы одно ошибочное отклонение (ложное открытие)) мощность уже больше никак не увеличить, то распространение получил контроль другой характеристики.

Рассмотрим долю ложных открытий FP среди всех открытий FP+TP, т.е. $\mathbb{E} FP/(FP+TP)$, называемую FDR (False Discovery Rate).
Более формально,
$$FDR = \mathbb{E} (FP/(FP+TP)\mid FP+TP>0)\, \mathbb{P}(FP+TP>0).$$

Процедуры, контролирующий FDR, являются stepdown:
\begin{itemize}
\item
Алгоритм Benjamini–Hochberg (Бенджамина-Хохберга?)гарантирует $FDR < \alpha$ в случае независимых или положительно коррелированных тестов.

1. Для заданного $\alpha$ найдите наибольшее $k$ такое, что $p_{(k)} \leq \frac{k}{m} \alpha$.

2. Отклоните нулевую гипотезу (т.е. сделайте открытие) для всех $H_{(i)}$ при $i = 1, \ldots, k$.

Геометрически это соответствует построению графика зависимости $p_{(k)}$ от $k$ (на осях $y$ и $x$ соответственно), проведению прямой через начало координат с наклоном $\frac{\alpha}{m}$ и объявлению открытий для всех точек слева, вплоть до последней точки, которая не находится над прямой.
\item
Алгоритм  Benjamini–Yekutieli (Беньямини-Екутиэли?) гарантирует $FDR < \alpha$ в общем случае.

Это уточнение изменяет порог в алгоритме Benjamini–Hochberg и находит наибольшее $k$ такое, что:
$p_{(k)} \leq \frac{k}{m \cdot c(m)} \alpha $.

Если тесты независимы или положительно коррелируют (как в процедуре Benjamini–Hochberg): $c(m)=1$.

При произвольной зависимости (включая случай отрицательной корреляции) $c(m)$ --- гармоническое число: $c(m) = \sum _{i=1} ^m \frac{1}{i}$.

\end{itemize}

Можно показать, что FWER $\geq$ FDR, поэтому, контролируя FWER, мы контролируем и FDR.

Если все гипотезы верные, то FWER=FDP (тогда FN = TP = 0).
...
\]
 ...
 Оказывается, $r_{i}^{(i)} = \frac{r_{i}}{1-h_{ii}}$. Поэтому есть такая связь между 
стьюдентизированными остатками и стандартизированными удаленными остатками (надо проверить, что формулы без опечаток)):\\
$$\frac{r_{i}}{\hat{\sigma}^{(i)}\sqrt{1-h_{ii}}} = \frac{r_{i}^{(i)}}{s(r_{i}^{(i)})}.$$
...
есть аутлаер:
\[
D_i = r_{M}^{2}(\hat{\bb},\hat{\bb}^{(i)};\cov\hat{\bb})=(\hat{\bb}-\hat{\bb}^{(i)})^{\T}\cov^{-1}(\hat{\bb})(\hat{\bb}-\hat{\bb}^{(i)})=\frac{1}{\sigma^{2}}(\hat{\bb}-\hat{\bb}^{(i)})\X^{\T}\X(\hat{\bb}-\hat{\bb}^{(i)})
\]
...
Рассмотрим соотношение расстояния по Куку и других мер:
\[
D_i = \frac { \sum_{j=1}^n \left( \widehat{y}_j - \widehat{y}_{j(i)} \right)^2 } {m \hat{\sigma}^{2}},
\]
\[
D_i = \frac{r_i^2}{m \hat{\sigma}^{2}}\left[\frac{h_{ii}}{(1-h_{ii})^2}\right]
\]
...
велико
\[
r_{M}^{2}(x_{i},\bar{\mathbf{x}};\S_{\mathbf{x}\mathbf{x}}) = (n - 1)(h_{ii} - \tfrac{1}{n}).
\]
...
%\end{document}
\part{Дополнительные материалы для курса `Вероятностные и статистические модели' (магистры 1 курса ПМИ, мат-мех)}
