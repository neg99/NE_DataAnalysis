...
\[
\FWER=\P \left(\text{отвергнута хотя бы одна верная гипотеза}\right).
\]
...
Пусть $\FWER(I) = \P\left(\bigcup_{i \in I} H_{(i)} \text{отвергнута} \right)$ --- FWER, если верны гипотезы с индексами $I$.
\begin{defn*}
\[
\weak\,\FWER = \FWER\left( \left\{ 1, \ldots, m \right\} \right)
\]
\end{defn*}

\begin{defn*}
\[
\strong\,\FWER=\max_{I \subset 1, \dots, m }\FWER(I).
\]
\end{defn*}
...
Каждая $H_{(i)}$ проверяется отдельно с уровнем значимости $\a_{1}$.
Задача сводится к тому, чтобы найти такое $\a_{1}$, что $\strong \, \FWER\leq\a$
для какого-то нужного предзаданного $\a$. Так как
\[
\FWER(I)=\P\left(\bigcup_{i \in I}H_{(i)}\text{ отверграется}\right)
\leq \sum_{i\in I}\P(H_{(i)}\text{ отвергается})=\left| I \right|\a_{1} \leqslant m \alpha_1 =\a,
\]
для любого $I$, то и $\strong \, \FWER \leqslant m \alpha_1$ 
\begin{rem*}
Из-за неравенства тест консервативный, т.е. $\strong \, \FWER\ll\a$. Значит
...
\end{defn*}
То есть каждую гипотезу нужно проверять с $\a_{1}$.

\subsection{Независимые гипотезы}

Если все критерии независимы, то возможно точно посчитать
$\weak \, \FWER$:
\begin{eqnarray*}
\weak \, \FWER & = & \P\left(\bigcup_{i=1}^{m}H_{0}^{(i)}\text{ отв}\right)=1-\P\left(\bigcap_{i=1}^{m}H_{0}^{(i)}\text{ не отв}\right)\\
 & = & 1-(1-\a_{1})^{m}=\a.
\end{eqnarray*}
Это позволяет ввести поправку, обеспечивающую $\weak \, \FWER = \alpha$
\begin{defn*}
\emph{Поправка Šidák}:
\[
\a_{1}=1-\sqrt[m]{1-\a}.
...
\item пусть $j$ --- минимальный индекс, такой, что
\[
p_{(j)}>\frac{\a}{m-j+1}
\]
 тогда $H_{(1)}, \ldots, H_{(j-1)}$ отвергаются, а $H_{(j)}, \ldots, H_{(m)}$ не отвергаются.
 \end{enumerate}

\begin{prop*}
$\strong \, \FWER\leq\a$.\end{prop*}
\begin{proof}
Пусть $I$ --- индексы (введенные в алгоритме) верных гипотез и 
$$
j = \min\limits_i \left\{i : i \in I, \, H_{(i)} \text{ отвергается}\right\}.
$$

Так как $j$ --- номер наименьшей неправильно отвергнутой гипотезы, то гипотезы $H_{(1)}, \ldots, H_{(j-1)}$ неверны (они правильно отвергнуты по определению $j$), а все верные гипотезы находятся среди $H_{(j)}, \ldots, H_{(m)}$, то есть
$$
\left| \left\{H_{(j)}, \ldots, H_{(m)} \right\}\right| \geqslant \left| I \right|,
$$
что можно записать как
$$
m - j +1 \geqslant \left| I \right|,
$$
$$
\frac{1}{m - j +1} \leqslant \frac{1}{\left| I \right|}.
$$
По построению алгоритма из того, что $H_{(i)}$ отвергается, следует, что $H_{(i-1)}$ также отвергается. Поэтому событие <<отвергнута гипотеза $H_{(i)}$>> включает в себя событие <<гипотезы $H_{(1)}, \ldots H_{(i)}$ отвергнуты>>. Из этого и из определения $j$ следует, что
\begin{align*}
	\FWER(I) = \P\left(\exists i \in I : H_{(i)} \text{ отвергается} \right) 
	= P\left(H_{(j)} \text{ отвергается}\right).
\end{align*}
По полученному ранее неравенству
\begin{align*}
	\FWER(I) &=
	P\left(H_{(j)} \text{ отвергается}\right) 
	= P\left(p_{(j)} < \frac{\alpha}{m - j +1}\right) \\ 
	&\leqslant P\left(p_{(j)} < \frac{\alpha}{\left| I \right| } \right) 
	\leqslant P\left(\bigcup\limits_{i \in I} \left( p_{(i)} < \frac{\alpha}{\left| I \right|} \right)\right) \\
	&\leqslant \sum\limits_{i \in I} P\left(p_{(i)} < \frac{\alpha}{\left| I \right| } \right) 
	= \left| I \right| \frac{\alpha}{\left| I \right| } = \alpha.
\end{align*}
То есть $\FWER(I) \leqslant \alpha$ для любого $I$, откуда следует, что $\strong \, \FWER \leqslant \alpha$.
\end{proof}
