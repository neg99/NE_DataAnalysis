...
\FWER_{\left\{ 1,\ldots,m\right\} } & = & \P\left(\bigvee_{i=1}^{m}H_{0}^{(i)}\text{ отв}\right)=1-\P\left(\bigwedge_{i=1}^{m}H_{0}^{(i)}\text{ не отв}\right)\\
 & = & 1-(1-\a_{1})^{m}=\a.
\end{eqnarray*}
Это позволяет ввести поправку на множественные тесты, делающая критерий точным:
\begin{defn*}
\emph{Поправка Šidák}:
\[
\a_{1}=1-\sqrt[m]{1-\a}.
...
\begin{align*}
	\weak \, \FWER(I) = \P\left(\exists i \in I : H_{(i)} \text{ отвергается} \right) 
...
\end{align*}
То есть $\weak \, \FWER(I) \leqslant \alpha$ для любого $I$, откуда следует, что $\strong \, \FWER \leqslant \alpha$.
\end{proof}
