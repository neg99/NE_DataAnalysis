...
\date{\$Revision: ... (NEG: UNDER CONSTRUCTION) \$}
...
Метод подстановки заключается в подстановке вместо неизвестного теоретического распределения известного эмпирического распределения. Например, вас интересует некоторая характеристика $f(\xi)$, а вы в качестве оценки предлагаете $\widehat{f(\xi)} = f(\xi_n)$, где $\hat\xi_n=\xi_n$ --- эмпирическая случайная величина.
...
Для $s^{2}=\frac{1}{n}\sum_{i=1}^n (x_i-\bar{x})^2$
имеем:
\begin{eqnarray*}
...
$\eta$ имеет распределение $\chi^{2}$ с $m$ степенями свободы $(\eta\sim\chi^{2}(m))$:
\[
...
$\alpha_I = \P_{H_0}(\x\in \mathcal{A}^{(\text{крит})}_\alpha) = \alpha$ ($\P_{H_0}$ --- вероятность, соответствующая предположению, что $H_0$ верна).
...
\item Если альтернативная гипотеза $H_{1}$ не конкретизирована, то $\Ascr_\a^{\text{(крит)}}$ следует выбрать
...
\[
\hat{\rho}(\xi,\eta)=\frac{\sum_{i=1}^n(x_i-\bar{x})(y_i-\bar{y})}{s_x s_y}=
\frac{\sum_{i=1}^n x_i y_i-\bar{x}\bar{y}}{s_x s_y}.
\]
...
%$\left(\eta,\xi\right)=\E\eta\xi$, причем $\hat{\eta}\in K=\left\{ \phi(\xi), \phi измерима \right\} =\left\{ \hat{\eta}:\sigma(\phi(\xi))\text{-измерима}\right\} $.
%По свойству УМО, вектор
%\[
...
\section{Происхождение и сравнение других мер зависимости}
...
Сравнивая полученные формулы, мы получаем, что коэффициент корреляции измеряет, насколько хорошо случайную величину $\eta$ можно приблизить линейной функцией от $\xi$, а корреляционное отношение --- произвольной (измеримой) функцией от $\xi$.
...
\part{Дисперсионный анализ \\ *NEG: не проверяла*}
...
Пусть $\xi,\eta\in L^{2}$ (с конечной дисперсии).\emph{ Парной линейной регрессией $\eta$
 на $\xi$} называется наилучшее среднеквадратичное приближение $h_{\b_{1}^{*},\b_{2}^{*}}(\xi)=\b_{1}^{*}\xi+\b_{2}^{*}$
в классе линейных по $\xi$ функций $K=\mathcal{L}=\left\{ \b_{1}\xi+\b_{2}\right\} $.
Иными словами,
\[
h_{\b_{1}^{*},\b_{2}^{*}}(\xi)=\argmin_{\b_{1},\b_{2}}\left\Vert \eta-h_{\b_{1},\b_{2}}(\xi)\right\Vert ^{2}
 =\argmin_{\b_{1},\b_{2}}\underbrace{\E(\eta-(\b_{1}\xi+\b_{2}))^{2}}_{\phi(\b_{1},\b_{2})}=\b_{1}^{*}\xi+\b_{2}^{*}.
\]
\end{defn*}
\begin{rem*}
...
\begin{claim*}
\label{claim:linreg-coeffs}$\b_{1}^{*},\b_{2}^{*}$ таковы, что для $y=h_{\b_{1}^{*},\b_{2}^{*}}(x)$
\[
\frac{y-\E\eta}{\sqrt{\D\eta}}=\rho\frac{x-\E\xi}{\sqrt{\D\xi}}.
\]
 Это уравнение задает линию регрессии. Иными словами,
\[
y=\underbrace{\rho\frac{\sqrt{\D\eta}}{\sqrt{\D\xi}}}_{\b_{1}^{*}}x+\underbrace{\E\eta-\rho\frac{\sqrt{\D\eta}}{\sqrt{\D\xi}}\E\xi}_{\b_{2}^{*}}.
\]
...
Пирсона
\[
\rho^{2}(\eta,\xi)=1-\frac{\phi(\b_{1}^{*},\b_{2}^{*})}{\D\eta}=1-\frac{\min_{\hat{\eta}\in \mathcal{L}}\E\left(\eta-\hat{\eta}\right)^{2}}{\D\eta},\quad\hat{\eta}:=h(\xi).
\]
\end{rem*}
\begin{defn*}
Линейная регрессия \emph{значима}, если $\beta_{1}^{*}\neq0\implies\rho\neq0$.
Значимость регрессии эквивалентна значимости предсказания по ней. В случае отсутствия угла наклона предсказание предсказание не имеет смысла, так как равно $\E\eta$ и не зависит от значений $\xi$. 
\end{defn*}
...
\[
\mathrm{SSR}=n\cdot\hat\phi(\b_{1}^{*},\b_{2}^{*})=\sum_{i=1}^{n}\left(y_{i}-\hat{y}_{i}\right)^{2},\quad\hat{y}_{i}=h_{\b_{1}^{*},\b_{2}^{*}}(x_{i}).
\]
