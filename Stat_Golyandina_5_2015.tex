...
\[
\FWER_{\left\{ 1,\ldots,m\right\} }=\P\left(\bigcup_{i=1}^{m}\{H_{0}^{(i)}\text{ отв}\}\right)\leq\sum_{i=1}^{m}\P(H^{(i)}\text{ отв})=m\a_{1}=\a\implies\a_{1}:=\frac{\a}{m}.
\]
...
\item \emph{(Псевдо-обратная по Муру-Пенроузу)} если по аналогии с $\A^{-1}=\A^{\T}\implies(\A^{-1}\A)^{\T}=\A^{\T}\left(\A^{-1}\right)^{\T}=\A^{-1}\A$,
...
%\newpage
Это биномиальное распределение с мат.ож. $np$. Находим границы: $P(np - c_1 < n\bar{x} < np + c_2) = \gamma$ (интервал покороче), отсюда получаем интервал для $p$.\\
