...
Отсюда правило двух сигма (вероятность быть на расстоянии от мат.ож. больше двух чигм примерно равна 0.05), правило трех сигм, правило шести сигм.
...
Тогда критерий --- это разбиение множества $V^n$ всевозможных значений выборки $\x$ на две области, критическую $\mathcal{A}^{(\text{крит})}_\alpha$ и доверительную $\mathcal{A}^{(\mbox{дов})}_\alpha$ так, что 
$\alpha_I = \P_{H_0}(\x\in \mathcal{A}^{(\text{крит})}_\alpha) = \alpha$ ($\P_{H_0}$ --- вероятность, соответствующая предположению, что $H_0$ верна).

При проверки гипотезы (уже после эксперимента), если выборка попала в критическую область $\mathcal{A}^{(\mbox{крит})}_\alpha$, то нулевая гипотеза отвергается, а если в доверительную $\mathcal{A}^{(\mbox{дов})}_\alpha$, то не отвергается (важно, что именно нет основания отвергнуть, но и принять нельзя).
...
Поэтому критическая область ${A}^{(\text{крит})}_\alpha$ выбирается так, чтобы
$\alpha_I=\P_{H_0} (t\in {A}^{(\text{крит})}_\alpha) = \alpha$. 
...
если $\forall\th$
\[
\P(\theta\in\left[b_{1},b_{2}\right])=\gamma,\quad\text{где }b_{1}=b_{1}(\mathbf{x}),b_{2}=b_{2}(\mathbf{x})\text{ --- статистики (функции от выборки, случайные величины).}
\]
\end{defn*}
\begin{rem*}
